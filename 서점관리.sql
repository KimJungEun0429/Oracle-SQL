...
FROM
(
    SELECT T1.BS_NAME, COUNT(T1.BS_CODE) AS COUNT
    ,RANK () OVER(ORDER BY COUNT(T1.BS_CODE) DESC) AS RNK
    FROM BSTORES T1, SELL_BOOK T2
    WHERE T1.BS_CODE = T2.BS_CODE
    GROUP BY T1.BS_NAME
)
WHERE RNK = 1
;
...
FROM
(
    SELECT T2.CR_CODE,T2.CR_NAME, SUM(T1.SB_CNT)
    ,RANK() OVER(ORDER BY SUM(T1.SB_CNT) DESC ) AS RNK
    FROM SELL_BOOK T1, CUSTOMERS T2
    WHERE T1.CR_CODE = T2.CR_CODE
    GROUP BY T2.CR_CODE,T2.CR_NAME
)
WHERE RNK = 1
;
        
-- 철학장르의 책을 가장 많이 구매한 회원

SELECT *
FROM
(
    SELECT T4.CR_NAME,T2.GR_CODE, T2.GR_NAME, SUM(T1.SB_CNT)
    ,RANK() OVER(ORDER BY SUM(T1.SB_CNT) DESC) AS RNK
    FROM   SELL_BOOK T1, GENRES T2, BOOKS T3, CUSTOMERS T4
    WHERE T3.BK_CODE = T1.BK_CODE
    AND T3.GR_CODE = T2.GR_CODE
    AND T4.CR_CODE = T1.CR_CODE
    AND T2.GR_CODE = 'GR01'
    GROUP BY T4.CR_NAME,T2.GR_CODE, T2.GR_NAME
)
WHERE RNK = 1
;
      
-- 1월 수입이 가장 많은 서점을 찾아보자 
SELECT *
FROM
(
    SELECT T1.BS_NAME, TO_CHAR(SUM(T2.SB_CNT * T3.BK_PRICE),'999,999,999') AS TOTAL_P
    ,RANK() OVER(ORDER BY TO_CHAR(SUM(T2.SB_CNT * T3.BK_PRICE),'999,999,999') DESC) AS RNK
    FROM BSTORES T1, SELL_BOOK T2, PRICE_BOOK T3
    WHERE T1.BS_CODE = T2.BS_CODE
    AND T2.BS_CODE = T3.BS_CODE AND T2.BK_CODE = T3.BK_CODE AND T2.HB_IDX = T3.HB_IDX
    AND TO_CHAR(T2.SB_OUTDATE,'MM') = 01
    GROUP BY T1.BS_NAME
)
WHERE RNK = 1
;
...
--회원들이 선호하는 서점을 찾아보자  --선호하는의 기준 : 많은 수량이 팔린인지 팔린횟수가 많은건지
   --많은 수량 기준이랑 팔린 횟수 기준 두 개로 풀어봐봥
SELECT *
FROM
(
    SELECT T1.BS_NAME, SUM(T2.SB_CNT)
    ,RANK() OVER(ORDER BY SUM(T2.SB_CNT) DESC) AS RNK
    FROM BSTORES T1, SELL_BOOK T2
    WHERE T1.BS_CODE = T2.BS_CODE
    GROUP BY T1.BS_NAME
)
...
FROM
(
    SELECT T1.BS_NAME, COUNT(T2.BS_CODE)
    ,RANK() OVER(ORDER BY SUM(T2.SB_CNT) DESC) AS RNK
    FROM BSTORES T1, SELL_BOOK T2
    WHERE T1.BS_CODE = T2.BS_CODE(+)
    GROUP BY T1.BS_NAME
...
--회원들이 선호하는 책 장르를 찾아보자 
SELECT *
FROM
(
    SELECT T2.GR_CODE, T2.GR_NAME , COUNT(T1.BS_CODE)
    ,RANK() OVER(ORDER BY COUNT(T1.BS_CODE) DESC) AS RNK
    FROM SELL_BOOK T1, GENRES T2, BOOKS T3
    WHERE T1.BK_CODE = T3.BK_CODE
    AND T3.GR_CODE = T2.GR_CODE
    GROUP BY T2.GR_CODE, T2.GR_NAME
)
WHERE RNK = 1
;
   
--가장 많은 판매량을 기록한 달을 찾아보자 
SELECT *
FROM
(
    SELECT TO_CHAR(SB_OUTDATE,'MM'), SUM(SB_CNT)
    ,RANK() OVER(ORDER BY SUM(SB_CNT) DESC) AS RNK
    FROM SELL_BOOK
    GROUP BY TO_CHAR(SB_OUTDATE,'MM')
)
WHERE RNK = 1
;
