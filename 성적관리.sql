...
(
    ...
    SELECT T1.PNAME, T3.SNAME, T2.ENAME
    FROM PROFESSORS T1, EMAMS T2, SUBJECTS T3
    WHERE T1.PID = T2.PID
    AND T3.SUBID = T2.SUBID
    ORDER BY T1.PNAME
    ;
    
    --2. 지역별 학생수 리스트를 보여주세요
    
    SELECT SADDR, COUNT(SADDR)
    FROM STUDENTS
    GROUP BY SADDR
    ;
    ...
    SELECT PADDR, COUNT(PADDR)
    FROM PROFESSORS
    GROUP BY PADDR
    ;
    
    --4  국어시험 1을 국어시험으로 치른 학생 리스트를 보여주세요(P001이 출제한 S001)
    SELECT T1.SNAME
    FROM STUDENTS T1, SCORES T2, PROFESSORS T3, SUBJECTS T4, EMAMS T5
    WHERE T1.SID = T2.SID
    AND T3.PID = T2.PID
    ...
    SELECT T1.SADDR, T3.SNAME, AVG(T2.SCORE), SUM(T2.SCORE)
    FROM STUDENTS T1, SCORES T2, SUBJECTS T3
    WHERE T1.SID = T2.SID
    AND T3.SUBID = T2.SUBID
    AND T1.SADDR = '부산'
    AND T3.SNAME = '영어'
    GROUP BY T1.SADDR, T3.SNAME
    ;
...
SELECT NVL(SCORE,0) FROM SCORES;
    ...
    SELECT T1.SNAME, COUNT(T2.SUBID)
    FROM STUDENTS T1, SCORES T2, SUBJECTS T3
    WHERE T1.SID = T2.SID
    AND T3.SUBID = T2.SUBID
    GROUP BY T1.SNAME
    ...
    SELECT T1.PGENDER, SUM(SCORE), AVG(SCORE)
    FROM PROFESSORS T1, EMAMS T2, SCORES T3
    WHERE T1.PID = T2.PID
    ...
    --11. 20대 여학생이 치른 시험의 총점과 평균을 구해주세요
        SELECT SUBSTR(T1.SAGE,1,1)||0, SUM(SCORE), AVG(SCORE)
        FROM STUDENTS T1, SCORES T2
        WHERE T1.SID = T2.SID
        AND SUBSTR(T1.SAGE,1,1) = '2'
        AND T1.SGENDER = 'F'
        GROUP BY SUBSTR(T1.SAGE,1,1)||0
        ;
    
    --12. 각 지역별로 학생들의 총점과 평균을 구해주세요
        SELECT SADDR, SUM(SCORE), AVG(SCORE)
        FROM STUDENTS T1, SCORES T2
        WHERE T1.SID = T2.SID
        GROUP BY SADDR
        ;
    --13. 각 과목(시험아님)별로 평균점수를 구해주세요
    SELECT T1.SNAME, AVG(SCORE)
    FROM SUBJECTS T1, SCORES T2, SUBJECTS T3
    ...
    SELECT T3.ENAME, T1.SNAME
    FROM STUDENTS T1, SCORES T2, EMAMS T3
    WHERE T1.SID = T2.SID(+)
    AND T3.SUBID = T2.SUBID(+)
    AND T2.SID IS NULL
    GROUP BY T3.ENAME, T1.SNAME
    ORDER BY T3.ENAME
    ;
    
    --17. 평균점수가 가장 높은 시험의 가장높은 점수를 받은 학생을 찾아주세요
    SELECT T3.ENAME, AVG(T2.SCORE)
    FROM STUDENTS T1, SCORES T2, EMAMS T3
    WHERE T1.SID = T2.SID
    AND T3.SUBID = T2.SUBID
    GROUP BY T3.ENAME
    ...
    SELECT T1.PADDR, MAX((SELECT T4.SNAME, AVG(T2.SCORE) FORM SCORES GROUP BY T4.SNAME))
    FROM PROFESSORS T1, SCORES T2, STUDENTS T3, SUBJECTS T4
    WHERE T1.PID = T2.PID
    AND T2.SID = T3.SID
    AND T4.SUBID = T2.SUBID
    GROUP BY T1.PADDR
    ...
    SELECT T4.SNAME, MAX(AVG(T2.SCORE))
    FROM PROFESSORS T1, SCORES T2, STUDENTS T3, SUBJECTS T4
    WHERE T1.PID = T2.PID
    AND T2.SID = T3.SID
    AND T4.SUBID = T2.SUBID
